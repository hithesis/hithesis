...
\section{如何做出符合规范的漂亮的图}[How to Make Figures that Meet the Specifications]
关于作图工具在后文\ref{drawtool}中给出一些作图工具的介绍，此处不多言。
此处以R语言和Tikz为例说明如何做出符合规范的图。

\subsection{Tikz作图举例}[Drawing with TikZ]
...
表格应具有三线表格式，推荐用~tabularray~宏包的写法，模板已装好它连同
booktabs~库，无需在导言区写任何东西。三线的线宽、单元格边距、表内字号行距由模板
...
排功能，需要长表格环境。推荐用~tabularray~的\verb|longtblr|环境：外参
数\verb|caption={中文表题}|，博士学位论文的英文表题写在\verb|caption-en|里，
与浮动表格的双语题注同一条链、同一个开关；续页重复表头（\verb|rowhead=1|）与续页
右上角的“表~x-x（续表）”都由模板默认给出，无需手写，所绘制的长表格的格式如
表~\ref{table2}~所示。老写法~longtable~宏包的\verb|longtable|环境仍然可用，
双语题注写\verb|\caption{中文题}[英文题]|，续表头需自己写\cs{endfirsthead}那一套。
...
首先给出这种情况下的一个例子如表~\ref{table3}~所示。新写法在~colspec~里把过长
的列标为~X，再用\verb|width|给出表格总宽度即可，写法见手册的表格一节；下面保
留~tabularx~的写法作为对照。
\begin{table}[htbp]
  \centering
...
注释三列，表格的总宽度可选为文本宽度，因此应该采用\verb|tabularx|环境。
由\verb|tabularx|环境生成的对公式中物理量符号进行注释的公式如式\eqref{eq:1}所示。
...
\noindent 由此方法生成的注释内容应紧邻待注释公式并置于其下方，因此不能将代码放
入\verb|table|浮动环境中。但此方法不能实现自动转页接排，可能会在当前页剩余空间不够
时，全部移动到下一页而导致当前页出现很大空白。因此在需要转页处理时，还请您手动将
需要转页的代码放入一个新的\verb|tabularx|环境中，将原来的一个\verb|tabularx|环境
拆分为两个\verb|tabularx|环境。

上面这套写法模板已经包成了\verb|eqdenote|环境，每条注释写一
个\cs{item}即可，引导词、列距、破折号与表后的行距都由环境处理，不必自己
写\verb|tabularx|。该环境底下用的是\verb|xltabular|，放不下会自动转页接排，
