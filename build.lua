...
-- .sty 是 hithesis.sty 与排版引擎 docxlike.sty 两个，显式列出来。
--
...
installfiles = {
  ...
  "hithesis.sty",
  "docxlike.sty",
  "hithesis.ist", "hithesis.bst", "hithesis.cfg",
  "docxlike.lua",
  "docxlike-font-metrics.lua",
  "docxlike-font-metrics.def",
...
--                      headheight 警告
--   69-toc-number-gap  同 25/31：目录条目字体首次加载的 fontspec 文案
--   70-tabularray      caption-en 外参绑 tabularray 2025 的 \DeclareTblrKeys，
--                      旧版按特性检测整块降级，断言值随之不同
--
...
if os.getenv("HITHESIS_TL_LEGACY") then
  excludetests = {
    ...
    "25-caption-short-title", "31-structure", "69-toc-number-gap",
    "70-tabularray",
  }
end
stdengine    = "xetex"
...
function typeset(file, dir)
  ...
  local steps = {
    typesetexe .. " " .. opts .. " " .. src,
    -- 索引用 upmendex：条目里有中文，makeindex 写出的分组标题不是合法 UTF-8，
    -- lualatex 读到就报错（见 Makefile 里 manual 目标的说明）
    "upmendex -s gind.ist -o " .. job .. ".ind " .. job .. ".idx",
